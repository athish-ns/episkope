...
class GroqService {
  constructor() {
    // Use environment variables for API key (supports both Vite and Create React App)
    this.apiKey = import.meta.env?.VITE_GROQ_API_KEY || process.env?.REACT_APP_GROQ_API_KEY;
  ...
  async summarizeText(text, maxLength = 200) {
    if (!this.apiKey) {
      throw new Error('Groq API key not configured. Please set VITE_GROQ_API_KEY or REACT_APP_GROQ_API_KEY environment variable.');
    }
          ...
          messages: [
            {
              ...
              Your job is to assess the severity of injuries and medical emergencies for a rehabilitation hospital based on descriptions provided.
              
              Assessment Scale:
              - 0-5: Low severity (minor injuries, stable condition, routine care needed)
              - 5-8.1: Moderate severity (moderate injuries, some risk, specialized care recommended)
              - 8.1-10: Extreme severity (serious injuries, high risk, immediate intensive care required)
